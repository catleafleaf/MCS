...
public final class CombatRenderingManager extends BaseEveryFrameCombatPlugin {
    private final static HashMap<CombatEngineLayers, List<Object>[]> _ENTITIES_MAP = new HashMap<>();
    private final static List<Object>[] _DIRECT_MAP = new ArrayList[BoxDatabase.ENTITY_TOTAL_TYPE_DIRECT];
    static {
        for (CombatEngineLayers layer : CombatEngineLayers.values()) {
            if (!_ENTITIES_MAP.containsKey(layer)) {
                ArrayList<Object>[] lists = new ArrayList[BoxDatabase.ENTITY_TOTAL_TYPE_NORMAL];
    ...
    public void init(CombatEngineAPI engine) {
        ...
        for (List<Object>[] lists : _ENTITIES_MAP.values()) {
            for (int i = 0; i < BoxDatabase.ENTITY_TOTAL_TYPE_NORMAL; i++) {
                for (Object entity : lists[i]) {
                    if (entity == null || ((RenderDataAPI) entity).hasDelete()) continue;
                    ((RenderDataAPI) entity).delete();
                }
                lists[i].clear();
            }
        }
        for (int i = 0; i < BoxDatabase.ENTITY_TOTAL_TYPE_DIRECT; i++) {
            for (Object entity : _DIRECT_MAP[i]) {
                if (entity == null || ((RenderDataAPI) entity).hasDelete()) continue;
                ((RenderDataAPI) entity).delete();
            }
            _DIRECT_MAP[i].clear();
        }
        this.engine.addLayeredRenderingPlugin(new BuiltinPlugin());
        if (Global.getCurrentState() == GameState.TITLE) {
            CampaignRenderingManager.clearRenderEntity();
        }
        Global.getLogger(CombatRenderingManager.class).info("'BoxUtil' Combat rendering manage invited!");
    }
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        for (List<Object> list : _DIRECT_MAP) {
    ...
    private static void forEntities(List<Object> list, float amount, boolean isPaused) {
        ...
        for (Iterator<Object> entitiesI = list.iterator(); entitiesI.hasNext();) {
            entity = (RenderDataAPI) entitiesI.next();
    ...
    private final static class BuiltinPlugin extends BaseCombatLayeredRenderingPlugin {
        private final FloatBuffer vanillaMatrix = BufferUtils.createFloatBuffer(16);
        private final FloatBuffer perspectiveMatrix = BufferUtils.createFloatBuffer(16);
        private CombatEngineAPI engine;
        private float lastFrameAmount = 0.0f;
        private float frameTime = 0.0f;
        private float frameTimePausedCheck = 0.0f;
        ...
        public void render(CombatEngineLayers layer, ViewportAPI viewport) {
            ...
            if (layer == RenderingUtil.getLowestCombatLayer()) {
                ...
                this.lastFrameAmount = this.engine.getElapsedInLastFrame();
                this.frameTime += this.lastFrameAmount;
                if (!this.engine.isPaused()) this.frameTimePausedCheck += this.lastFrameAmount;
                this.idle = true;
                this.needAA = false;
            }
            ...
            List<Object> commonList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_COMMON];
            List<Object> spriteList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_SPRITE];
            List<Object> curveList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_CURVE];
            List<Object> segmentList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_SEGMENT];
            List<Object> trailList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_TRAIL];
            List<Object> flareList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_FLARE];
            List<Object> customList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_CUSTOM];
            List<Object> textList = _ENTITIES_MAP.get(layer)[BoxEnum.ENTITY_TEXT];
            List<Object> distortionList = _DIRECT_MAP[BoxEnum.ENTITY_DISTORTION];
            List<Object> illuminantList = _DIRECT_MAP[BoxEnum.ENTITY_ILLUMINANT];
            ...
            if (layer == RenderingUtil.getHighestCombatLayer()) {
                ...
                if (!distortionList.isEmpty()) {
                    if (BoxConfigs.isDistortionEnable() && notMultiPass) {
                        ...
                        instanceBit = 1;
                        for (Iterator<Object> entitiesI = distortionList.iterator(); entitiesI.hasNext();) {
                            entity = (RenderDataAPI) entitiesI.next();
                            if (entity == null) {
                                entitiesI.remove();
                                continue;
            ...
            if (!commonList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    for (Iterator<Object> entitiesI = commonList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
                        ...
                        instance = (InstanceRenderAPI) entity;
                        boolean entityColorMode = material.isIgnoreIllumination() || globalColorMode;
            ...
            if (!spriteList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    program.putUniformSubroutines(GL20.GL_VERTEX_SHADER, 0, vertexSub);
                    instanceBit = 4;
                    for (Iterator<Object> entitiesI = spriteList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
            ...
            if (!curveList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    for (Iterator<Object> entitiesI = curveList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
            ...
            if (!segmentList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    program.active();
                    GL40.glPatchParameteri(GL40.GL_PATCH_VERTICES, 2);
                    for (Iterator<Object> entitiesI = segmentList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
            ...
            if (!trailList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    program.active();
                    for (Iterator<Object> entitiesI = trailList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
            ...
            if (!flareList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    instanceBit = 1;
                    for (Iterator<Object> entitiesI = flareList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
            ...
            if (!customList.isEmpty()) {
                if (notMultiPass) {
                    for (Iterator<Object> entitiesI = customList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
            ...
            if (!textList.isEmpty()) {
                if (notMultiPass) {
                    ...
                    program.active();
                    for (Iterator<Object> entitiesI = textList.iterator(); entitiesI.hasNext();) {
                        entity = (RenderDataAPI) entitiesI.next();
                        if (entity == null) {
                            entitiesI.remove();
                            continue;
        ...
        private void blendCheckA(RenderDataAPI entity) {
            switch (entity.getBlendState()) {
                ...
                case 2: {
                    GL14.glBlendFuncSeparate(entity.getBlendColorSRC(), entity.getBlendColorDST(), entity.getBlendAlphaSRC(), entity.getBlendAlphaDST());
        ...
        private void removeCheck(Iterator<Object> iterator, ControlDataAPI data, RenderDataAPI entity) {
        ...
        private void glDisabledIterator(List<Object> list) {
            RenderDataAPI entity;
            for (Iterator<Object> entitiesI = list.iterator(); entitiesI.hasNext();) {
                entity = (RenderDataAPI) entitiesI.next();
                if (entity == null) {
    ...
    public static byte addEntity(byte target, @NotNull RenderDataAPI entity) {
        ...
        if (layer instanceof CombatEngineLayers) {
